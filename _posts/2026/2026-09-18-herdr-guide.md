...
![참고 영상 시연 화면 — claude·kimi·claude 2는 "작업 중", codex는 "승인 대기 38분째"로 표시된 사이드바](/assets/img/posts/herdr-guide/demo-agents-status.png)

위 화면이 실제 영상에서 시연한 장면이다. codex 하나만 38분째 승인 대기 상태로 멈춰 있는 게 한눈에 보인다 — 나머지 에이전트가 작업 중이어도 놓치지 않고 바로 확인할 수 있다.
...
![참고 영상 시연 화면 — tmux는 페인을 닫거나 분리하면 그걸로 끝이지만, herdr는 서버가 세션을 계속 들고 있어 페인을 닫아도 살아있다는 비교 다이어그램](/assets/img/posts/herdr-guide/demo-persistence.png)

영상은 tmux의 "닫으면 끝"인 구조와 herdr 서버가 세션 1·2를 계속 들고 있는 구조를 이렇게 대비시켜 보여준다.
...
![참고 영상 시연 화면 — 왼쪽 페인에서 Claude Code가 herdr 스킬로 codex 페인을 열고, 오른쪽 페인에서 OpenAI Codex가 계획서(PLAN.md)를 검토하는 실제 화면](/assets/img/posts/herdr-guide/demo-claude-codex.png)

실제 시연 장면이다. 왼쪽은 Claude Code가 `herdr agent prompt codex-reviewer` 명령으로 오른쪽에 새 페인을 열어 Codex를 띄우는 과정, 오른쪽은 그렇게 열린 Codex CLI가 스크래치패드에 저장된 계획서를 받아 검토하는 과정이다.
