...
정보, `~/Documents/ai-docs/llm-wiki` 등 회사 위키에서 복사해온 내용, 특정
사내 프로젝트의 코드/설정/장애 기록 등) ingest 전에 반드시 사용자에게
"이거 공개 저장소에 들어가도 되는 내용이 맞냐"고 먼저 확인한다. **판단이
애매하면 통과시키지 말고 반드시 물어본다** — 기본값은 거부다.

## 개인정보/시크릿 유입 방지 (이 저장소는 GitHub 공개 저장소)

`wiki/`는 `_config.yml`의 `exclude`로 Jekyll **빌드에서만** 빠질 뿐, git에는
그대로 커밋된다 — 즉 GitHub 저장소 페이지에서는 누구나 볼 수 있다. "빌드 안
되니까 안 보인다"는 착각하지 말 것.

ingest하려는 내용에 다음이 섞여 있으면 **그 부분을 빼고 저장하거나, 어느
수준까지 남길지 사용자에게 먼저 확인한다** (기본값은 제외):

- API 키/토큰/비밀번호/개인키, 인증서, `.env` 값 등 자격증명
- 본인 또는 타인의 전화번호, 주소, 주민번호류 식별자, 계좌/카드번호
- 타인의 실명·이메일·연락처 (본인 것은 이미 공개 프로필에 있는 정보면 무방,
  헷갈리면 확인)
- 특정 개인을 특정할 수 있는 사진/스크린샷(얼굴, 문서, 화면에 뜬 개인정보)
- 건강/재정 등 민감한 개인 신상 정보

이 확인은 위 "회사/업무 자료 유입 방지"와 별개로 **항상** 거친다 — 회사
자료가 아니어도(예: 순수 개인 메모) 위 항목이 있으면 걸러야 한다.
...
3. 이후는 `.claude/skills/post/SKILL.md`의 "작성" 절차를 그대로 따른다.
   ...
   - 생성된 포스트에 위키 페이지로 돌아가는 링크는 넣지 않는다 (`wiki/`는
     빌드 제외 대상이라 죽은 링크가 됨)
   - 본문은 비워두지 않는다 — 2번에서 재구성한 내용을 그대로 채운다
     (post 스킬의 "본문은 비워둔다" 규칙은 이 경우 적용하지 않음).
   - `wiki/raw/`의 원본 세부 내용(고유명사, 수치, 내부 식별자)이 포스트에
     그대로 노출되지 않는지 확인한다 — 위키 페이지 수준의 요약을 유지하고,
     애매하면 발행 전에 사용자에게 확인한다.
...
6. 위 "개인정보/시크릿 유입 방지" 항목이 이미 들어와 있는 건 아닌지
   (`wiki/raw/`, `wiki/concepts/`, `wiki/decisions/` 전체 대상) — 발견되면
   lint 리포트에서 가장 먼저, 별도로 강조해서 보고한다
